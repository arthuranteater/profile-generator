...
const opener = async (file, app) => {
  await open(file, { app: app })
}
...
const createPDF = html => {
  console.log(html)
  htmlPDF.create(html).toFile('./profile.pdf', function (err, res) {
    if (err) {
      console.log(err)
    }
...
promptUser()
  .then(({ color, github }) => {
    ...
    getUser(github).then(response => getTotalStars(github).then(stars => {
      ...
      .then(html => {
        ...
        opener(path.join(process.cwd(), "profile.pdf"), 'google chrome')
      })
      .catch(function (err) {
        console.log(err);
      })
